...
describe('NewSession exercise suggestions', () => {
  ...
  it('fills only first series from suggestion', async () => {
    ...
    await waitFor(() => {
      ...
      expect((secondInputs[0] as HTMLInputElement).value).toBe('')
      expect((secondInputs[1] as HTMLInputElement).value).toBe('')
    })
  })

  it('clears first series when no suggestion is available', async () => {
    vi.mocked(api.getExercises).mockResolvedValue([
      { id: 41, name: 'Wykroki z hantlami', muscle_groups: '["nogi"]' },
    ...
    await waitFor(() => {
      const firstExercise = container.querySelector('.ex-block')
      const firstRowInputs = firstExercise!.querySelectorAll('.series-row')[0].querySelectorAll('input')

      expect((firstRowInputs[0] as HTMLInputElement).value).toBe('')
      expect((firstRowInputs[1] as HTMLInputElement).value).toBe('')
    })
  })
})
